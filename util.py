...
def check_sleep_period(period: str, logger) -> None:
    ...
    if not period.isdigit():
        logger.error(
            "Неверно указан период синхронизации, это должно быть "
            "целое число(измерение в секундах)."
        )
        sys.exit(1)
...
def check_path_exists(path: str, logger) -> None:
    ...
    if not check:
        logger.error(f"Указанный путь к {path} не существует. "
                     f"Введите корректный путь.")
        sys.exit(1)
...
def func_error_logging(name: str, args: tuple, logger) -> None:
    ...
    :param args: Кортеж с аргументами переданными в функцию,
        нужен для получения имени файла.
    :param logger: Логгер для записи логов.
    """
    if name == "cloud_load" and len(args) == 3:
        logger.error(
            f"Файл {args[-1]} не был сохранен. Проверьте соединение с "
            f"интернетом."
        )

    elif name == "cloud_load" and len(args) == 4:
        logger.error(
            f"Файл {args[-2]} не был перезаписан. Проверьте соединение с "
            f"интернетом."
        )

    elif name == "delete_file":
        logger.error(
            f"Файл {args[-1]} не был удален. Проверьте соединение с "
            f"интернетом."
        )
